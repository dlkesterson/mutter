...
**Backend (src-tauri/):**
- Rust + Tauri v2
- Candle 0.8 (ML framework for Whisper + BERT)
- Tokio (async runtime)
- Tauri plugins:
...
│   ├── QuickCapture.tsx              # Quick note capture
│   ├── Omnibox.tsx                   # Command palette (Cmd+K)
│   ├── CrdtSpike.tsx                 # CRDT experimentation
│   ├── dialogs/
...
- `docs/VAULT-METADATA-CRDT.md` - Vault metadata schema (if exists)
