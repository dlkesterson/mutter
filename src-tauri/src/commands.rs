...
/// List all tasks from Agent-Tracker
#[tauri::command]
pub async fn list_agent_tracker_tasks() -> Result<Vec<AgentTrackerTask>, String> {
    log::info!("Listing Agent-Tracker tasks");

    // Get tracker path from environment or use default
    let tracker_project_path = std::env::var("AGENT_TRACKER_PATH")
        .unwrap_or_else(|_| {
            // Default: look for agent-tracker in parent directories or home
            let home = std::env::var("HOME").unwrap_or_else(|_| "/home/linuxdesktop".to_string());
            format!("{}/Code/agent-tracker", home)
        });

    let tasks_dir = std::path::Path::new(&tracker_project_path)
        .join(".agent-tracker")
        .join("tasks");

    if !tasks_dir.exists() {
        log::warn!("Agent-Tracker tasks directory not found at: {:?}", tasks_dir);
    ...
    for entry in entries {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            match std::fs::read_to_string(&path) {
                Ok(contents) => {
                    match serde_json::from_str::<AgentTrackerTask>(&contents) {
    ...
    log::info!("Loaded {} tasks from Agent-Tracker", tasks.len());
    Ok(tasks)
}

/// Update checkbox in source file based on task status
#[tauri::command]
pub async fn sync_task_checkbox(
    source_file: String,
    task_description: String,
    is_completed: bool,
) -> Result<String, String> {
    log::info!(
        "Syncing checkbox for task '{}' in file: {}",
        task_description,
        source_file
    );
    ...
    for line in content.lines() {
        ...
        if (trimmed.starts_with("- [ ] ") || trimmed.starts_with("- [x] "))
            && trimmed.contains(&task_description)
        {
            // Update the checkbox based on task status
            let checkbox = if is_completed { "- [x] " } else { "- [ ] " };
            let indentation = &line[0..line.len() - trimmed.len()];
            let task_text = if trimmed.starts_with("- [ ] ") {
                &trimmed[6..]
            } else {
                &trimmed[6..]
            };

            updated_content.push_str(&format!("{}{}{}\n", indentation, checkbox, task_text));
            found = true;
        } else {
    ...
    log::info!("Successfully synced checkbox for task: {}", task_description);
    Ok(format!("Checkbox updated to: {}", if is_completed { "done" } else { "todo" }))
}

/// Check if Gattai daemon is running by reading heartbeat file
#[tauri::command]
pub async fn check_gattai_heartbeat() -> Result<Option<GattaiHeartbeat>, String> {
    log::info!("Checking Gattai heartbeat");

    // Look for agent-tracker in default location
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/linuxdesktop".to_string());
    let tracker_path = format!("{}/Code/agent-tracker", home);
    let heartbeat_path = std::path::Path::new(&tracker_path)
        .join(".gattai")
        .join(".heartbeat");

    if !heartbeat_path.exists() {
        log::debug!("Gattai heartbeat file not found");
        return Ok(None);
    }

    match std::fs::read_to_string(&heartbeat_path) {
        Ok(contents) => {
            match serde_json::from_str::<GattaiHeartbeat>(&contents) {
                Ok(heartbeat) => {
                    log::debug!("Gattai heartbeat found: {:?}", heartbeat);
                    Ok(Some(heartbeat))
                }
                Err(e) => {
                    log::warn!("Failed to parse Gattai heartbeat: {}", e);
                    Ok(None)
                }
            }
        }
        Err(e) => {
            log::warn!("Failed to read Gattai heartbeat file: {}", e);
            Ok(None)
        }
    }
}
...
#[tauri::command]
pub async fn check_syncthing_status(vault_path: Option<String>) -> Result<bool, String> {
    log::info!("Checking Syncthing status");

    // Method 1: Check for .stfolder marker in vault (if vault path provided)
    if let Some(vault) = vault_path {
        let stfolder_path = std::path::Path::new(&vault).join(".stfolder");
        if stfolder_path.exists() {
            log::debug!("Found .stfolder marker, Syncthing is configured");

            // Method 2: Try to ping Syncthing REST API
            match check_syncthing_api().await {
                Ok(true) => {
                    log::debug!("Syncthing API is responding");
                    return Ok(true);
                }
                Ok(false) => {
                    log::debug!("Syncthing configured but API not responding");
                    return Ok(false);
                }
                Err(e) => {
                    log::debug!("Error checking Syncthing API: {}", e);
                    // Fall through to check process
                }
            }
        }
    }

    // Method 3: Check if syncthing process is running (fallback)
    match std::process::Command::new("pgrep").arg("syncthing").output() {
        Ok(output) => {
            let is_running = output.status.success() && !output.stdout.is_empty();
            log::debug!("Syncthing process check: {}", is_running);
            Ok(is_running)
        }
        Err(e) => {
            log::warn!("Failed to check Syncthing process: {}", e);
            Ok(false)
        }
    }
}

/// Helper: Check Syncthing REST API
async fn check_syncthing_api() -> Result<bool, String> {
    // Try default Syncthing API endpoint
    let url = "http://localhost:8384/rest/system/ping";
