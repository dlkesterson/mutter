...
function SuggestionPanel({
	...
	const style = position
		? {
				...
				left: Math.max(
					100,
					Math.min(position.x, window.innerWidth - 300),
				),
				top: position.y + 24,
				zIndex: 1000,
			}
	...
	return (
		<div
			className='min-w-50 max-w-75 p-2 bg-surface/95 backdrop-blur-md border border-border/30 rounded-lg shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-200'
			style={style}
		>
			{/* Primary tier */}
			{primary.length > 0 && (
			...
			{secondary.length > 0 && (
				<div
					className={`flex flex-col gap-0.5 ${primary.length > 0 ? 'mt-2 pt-2 border-t border-border/20' : ''}`}
				>
 ...
 */
export function useCursorScreenPosition(
	view: any | null,
