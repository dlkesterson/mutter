...
export function GraphDialog({ open, onOpenChange, onNavigate }: GraphDialogProps) {
  ...
  const { graphData, loading, nodeCount, edgeCount } = useFullGraphData({ showOrphans });
  ...
  const filteredData = useMemo(() => {
    ...
    const matchingNodeIds = new Set(
      graphData.nodes
        .filter((n) => n.name.toLowerCase().includes(query) || n.relPath.toLowerCase().includes(query))
        .map((n) => n.id)
    );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-[95vw] max-h-[95vh] w-[95vw] h-[95vh] p-0 gap-0"
        style={{ backgroundColor: graphColors.background }}
      >
        {/* Header */}
        <DialogHeader className="p-4 border-b border-border flex-shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-lg font-medium">Vault Graph</DialogTitle>
            <div className="flex items-center gap-4">
              {/* Stats */}
              <div className="text-xs text-muted-foreground">
                <span className="text-foreground font-medium">{nodeCount}</span> notes ·{' '}
                <span className="text-foreground font-medium">{edgeCount}</span> links ·{' '}
                <span className="text-foreground font-medium">{orphanCount}</span> orphans
              </div>
            </div>
          </div>
          ...
          <div ref={containerRef} className="flex-1 min-w-0">
            {loading ? (
              ...
              <GraphView
                ...
                activeNodeId={filteredData.nodes.find((n) => n.isCurrent)?.id ?? null}
                callbacks={{
