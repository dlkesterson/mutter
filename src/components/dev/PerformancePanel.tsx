...
export function PerformancePanel() {
  ...
  useEffect(() => {
    const handleQuery = (e: CustomEvent<{ executionTimeMs: number; timing: QueryTiming; noteCount: number }>) => {
      setMetrics((prev) => ({
        ...prev,
        queryTimes: [...prev.queryTimes.slice(-19), e.detail.executionTimeMs],
        queryTimings: [...prev.queryTimings.slice(-19), e.detail.timing],
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {/* Expanded view */}
      {isExpanded && (
        <div className="bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl p-4 min-w-[280px] text-xs font-mono">
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-bold text-zinc-200">Performance Metrics</h3>
          ...
          <div className="space-y-2 text-zinc-400">
            ...
            <div className="border-b border-zinc-800 pb-2">
              <div className="text-zinc-300 font-medium mb-1">Query Execution</div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                <span>Avg:</span>
                <span className={avgTime < 100 ? 'text-green-400' : 'text-yellow-400'}>
                  {avgTime.toFixed(1)}ms
                </span>
                <span>Max:</span>
                <span className={maxTime < 100 ? 'text-green-400' : 'text-yellow-400'}>
                  {maxTime.toFixed(1)}ms
                </span>
                <span>Last:</span>
                <span>{metrics.queryTimes.at(-1)?.toFixed(1) || '-'}ms</span>
                <span>Samples:</span>
            ...
            {lastTiming && (
              <div className="border-b border-zinc-800 pb-2">
                <div className="text-zinc-300 font-medium mb-1">Last Query Breakdown</div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                  <span>Index lookup:</span>
                  <span>{lastTiming.indexLookupMs.toFixed(2)}ms</span>
                  <span>Filtering:</span>
                  <span>{lastTiming.filterMs.toFixed(2)}ms</span>
                  <span>Sorting:</span>
                  <span>{lastTiming.sortMs.toFixed(2)}ms</span>
                  <span>Total:</span>
                  <span>{lastTiming.totalMs.toFixed(2)}ms</span>
            ...
            {/* Memory */}
            <div>
              <div className="text-zinc-300 font-medium mb-1">Memory</div>
            ...
            <div className="pt-2 border-t border-zinc-800 text-zinc-500">
              <span className="text-green-400">Green</span> = &lt;100ms target met
            </div>
          </div>
        </div>
      )}
