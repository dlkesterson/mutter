 ...
 */
export function SearchPanel({ vaultPath, llmSettings, onNavigate }: SearchPanelProps) {
	...
	const allSuggestions = [
		...matchingRecentQueries.map((q) => ({ type: 'recent' as const, value: q })),
			...
			<div className="flex-1 flex flex-col overflow-hidden p-4 space-y-4">
					...
					<>
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								{dslResult && (
									<span className="text-xs text-muted-foreground">
										{dslResult.totalCount} result{dslResult.totalCount !== 1 ? 's' : ''} (
										{dslResult.executionTimeMs.toFixed(1)}ms)
									</span>
								)}
							</div>
							<button
							...
							{showSuggestions && allSuggestions.length > 0 && (
								<div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border rounded shadow-lg z-10 overflow-hidden">
									{allSuggestions.map((item, idx) => (
										...
										>
											{item.type === 'recent' && (
												<svg className="text-muted-foreground flex-shrink-0" width="12" height="12" viewBox="0 0 12 12" fill="currentColor">
													<circle cx="6" cy="6" r="5" fill="none" stroke="currentColor" />
													<path d="M6 3v3l2 1" stroke="currentColor" fill="none" />
												</svg>
											)}
						...
						{/* Query Description */}
						<p className="text-xs text-muted-foreground">{dslDescription}</p>
							...
							<div className="flex-1 overflow-auto">
								<ul className="space-y-1">
									{dslResult.notes.map((note) => (
										<NoteResultItem key={note.id} note={note} onNavigate={onNavigate} />
									))}
								</ul>
							</div>
						)}
						...
						{/* No Results */}
						{dslResult && dslResult.notes.length === 0 && dslQuery && (
						...
						{!dslResult && !dslQuery && (
							<div className="flex-1 flex flex-col items-center justify-center space-y-4">
								...
								<div className="text-xs text-muted-foreground space-y-1">
									<p><code className="px-1 py-0.5 bg-muted rounded">type:project</code> — Supertag</p>
									<p><code className="px-1 py-0.5 bg-muted rounded">tag:work</code> — Markdown tag</p>
									<p><code className="px-1 py-0.5 bg-muted rounded">linked:[[Note]]</code> — Links to</p>
								</div>
							</div>
						)}
				...
				{mode === 'ai' && (
					<>
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-muted-foreground">
								{indexSize > 0 ? `${indexSize} notes indexed` : 'Not indexed'}
							</span>
							<div className="flex items-center gap-2">
								{indexSize > 0 && (
									<button
						...
						<form onSubmit={handleAiSubmit} className="space-y-2">
							<textarea
								...
								className="w-full px-3 py-2 border border-border bg-background rounded text-sm resize-none min-h-[60px] focus:outline-none focus:ring-2 focus:ring-ring"
								disabled={aiLoading}
								rows={2}
							/>
							<button
								type="submit"
								disabled={aiLoading || !aiInput.trim() || indexSize === 0}
						...
						{/* Empty State */}
						{!aiResult && !aiError && !aiLoading && indexSize > 0 && (
							<div className="flex-1 flex items-center justify-center">
								<p className="text-sm text-muted-foreground text-center">
									Ask a question about your notes
									<br />
									<span className="text-xs">e.g., "Summarize my project notes"</span>
								</p>
							</div>
						)}
					</>
